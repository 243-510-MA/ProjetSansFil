...
ASSIGN_COORDINATOR_SHORT_ADDRESS:                                                            
                            ...
                            else
                            {
                                ...
                                if(MACRxPacket.Payload[0] >= 32 )
                                {
                                    rxMessage.flags.bits.firsthop = 1;
                                }
                                else
                                {
                                    rxMessage.flags.bits.firsthop = 0;
                                }
...
NO_INDIRECT_MESSAGE:            
    ...
    void DumpConnection(uint8_t index)
    {
        ...
        if( index == 0xFF )
        {
            ...
            for(i = 0; i < MY_ADDRESS_LENGTH; i++)
            {
                CONSOLE_PutString(myLongAddress[MY_ADDRESS_LENGTH-1-i]);
            }
            Printf("  Short Addr: ");
            CONSOLE_PutString(myShortAddress.v[1]);
            CONSOLE_PutString(myShortAddress.v[0]);
            Printf("  PANID: ");
            CONSOLE_PutString(myPANID.v[1]);
            CONSOLE_PutString(myPANID.v[0]);
            Printf("  Channel: ");
        ...
        if( index == 0xFF )
        {
            for(i=0;i<CONNECTION_SIZE;i++)
            {
                if(ConnectionTable[i].status.bits.isValid)
                {
                    ...
                    {
                        //this is a neighbor
                        CONSOLE_PutString(ConnectionTable[i].PANID.v[1]);
                        CONSOLE_PutString(ConnectionTable[i].PANID.v[0]);
                        CONSOLE_Put(' ');
                        CONSOLE_Put(' ');
                        CONSOLE_PutString(ConnectionTable[i].AltAddress.v[1]);
                        CONSOLE_PutString(ConnectionTable[i].AltAddress.v[0]);
                        CONSOLE_Put(' ');
                    }
                    else
                    {
                    ...
                    if(ConnectionTable[i].status.bits.longAddressValid)
                    {
                        ...
                        for(j = 0; j < MY_ADDRESS_LENGTH; j++)
                        {
                            CONSOLE_PutString(ConnectionTable[i].Address[MY_ADDRESS_LENGTH-1-j]);
                        }
                    }
                        ...
                        for(j = 0; j < ADDITIONAL_NODE_ID_SIZE; j++)
                        {
                            CONSOLE_PutString(ConnectionTable[i].PeerInfo[j]);
                        }
                    #endif
        ...
        else
        {
            if(ConnectionTable[index].status.bits.isValid)
            {
                ...
                {
                    //this is a neighbor
                    CONSOLE_PutString(ConnectionTable[index].PANID.v[1]);
                    CONSOLE_PutString(ConnectionTable[index].PANID.v[0]);
                    CONSOLE_Put(' ');
                    CONSOLE_Put(' ');
                    CONSOLE_PutString(ConnectionTable[index].AltAddress.v[1]);
                    CONSOLE_PutString(ConnectionTable[index].AltAddress.v[0]);
                    CONSOLE_Put(' ');
                }
                else
                {
                ...
                if(ConnectionTable[index].status.bits.longAddressValid)
                {
                    ...
                    for(j = 0; j < MY_ADDRESS_LENGTH; j++)
                    {
                        CONSOLE_PutString(ConnectionTable[index].Address[MY_ADDRESS_LENGTH-1-j]);
                    }
                }
                    ...
                    for(j = 0; j < ADDITIONAL_NODE_ID_SIZE; j++)
                    {
                        CONSOLE_PutString(ConnectionTable[index].PeerInfo[j]);
                    }
                #endif
